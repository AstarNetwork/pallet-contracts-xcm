...
pub trait XCMExtension {
    ...
    #[ink(extension = 0x00A0000, handle_status = false, returns_result = false)]
    fn prepare_execute(xcm: VersionedXcm<()>) -> u64;

    #[ink(extension = 0x00A0001, handle_status = false, returns_result = false)]
    fn execute();

    #[ink(extension = 0x00A0002, handle_status = false, returns_result = false)]
    fn prepare_send(dest: VersionedMultiLocation, xcm: VersionedXcm<()>) -> VersionedMultiAsset;

    #[ink(extension = 0x00A0003, handle_status = false, returns_result = false)]
    fn send();

    #[ink(extension = 0x00A0004, handle_status = false, returns_result = false)]
    fn new_query() -> u64;

    #[ink(extension = 0x00A0005, handle_status = true, returns_result = false)]
...
mod xcm_contract_poc {
    ...
    impl XcmContractPoC {
        ...
        #[ink(message)]
        pub fn send_message(&mut self, para_id: u32, call: Vec<u8>, weight: u64) {
